...
# bootstraped data for 95%-confidence intervals of empirical eigenvalues, index loadings, and
# correlations
...
confint <- apply(pervarboot,MARGIN=2,FUN=quantile, probs=c(0.025,0.975)) # confidence intervals of percentage of variation
...
for (j in 1:length(eigenvalues)) {
	for (k in 1:dim(x)[2]) {
		...
		confintindboot<- rbind (confintindboot, quantile (indices, probs=c(0.025,0.975)))
		}
	}
...
if (corr==T) {
	...
	for (j in 1:length(eigenvalues)) {
		for (k in 1:dim(x)[2]) {
			...
			confintcorboot<- rbind (confintcorboot, quantile (indices, probs=c(0.025,0.975)))
		}
	}
}
...
confintperm <- apply (pervarperm, MARGIN=2, FUN=quantile, probs=c(0.025,0.975))
...
for (j in 1:length(eigenvalues)) {
	for (k in 1:dim(x)[2]) {
		...
		confintind<- rbind (confintind, quantile (indices, probs=c(0.025,0.975)))
...
for (j in 1:length(eigenvalues)) {
	for (k in 1:dim(x)[2]) {
		...
		confintcor<- rbind (confintcor, quantile (cors, probs=c(0.025,0.975)))
...
h <- hist(Psi,plot=FALSE)
h$density = h$counts/sum(h$counts)*100
plot(h,freq=FALSE, col="gray45", xlab="Psi", xlim=c(0, max(max(Psi), Psiobs)), ylab="Percentage of permutations", main="")
arrows(x0=Psiobs, y0=max(h$density)/10, y1=0, col="red", lwd=2, length=0.1)
legend("topright", legend=c("Null distribution","Empirical value"), fill=c("gray45","red"), box.lty=0)

# plot of empirical and randomized Phi

h <- hist(Phi,plot=FALSE)
h$density <- h$counts/sum(h$counts)*100
plot(h,freq=FALSE, col="gray45", xlab="Phi", xlim=c(0, max(max(Phi), Phiobs)), ylab="Percentage of permutations", main="")
arrows (x0=Phiobs, y0=max(h$density)/10, y1=0, col="red", lwd=2, length=0.1)
...
if (Psiprob < alpha & Phiprob < alpha) { # test PC axes if both Psi and Phi are significant
	...
	lines (apply(pervarperm,MARGIN=2,FUN=mean), type="b", pch=19, lty="dashed", col="gray45")
	suppressWarnings( arrows (x0=c(1:length(eigenvalues)), y0=confint[2,], y1=confint[1,], code=3, angle=90, length=0.05, col="red"))
	suppressWarnings( arrows (x0=c(1:length(eigenvalues)), y0=confintperm[2,], y1=confintperm[1,], code=3, angle=90, length=0.05, col="gray45"))
...
# plot of bootstrapped and randomized index loadings for significant PC's only
	...
	for (i in 1:sigaxes) {
		...
		if (i %in% seq(2,max(2,sigaxes),4)) {	# if sigaxes is 2, 6, 10,... make another window to display more plots
			...
			lines (meanind[k:(k-1+dim(x)[2])], type="p", pch=19, col="gray45")
			suppressWarnings( arrows (x0=c(1:dim(x)[2]), y0=confintindboot[k:(k-1+dim(x)[2]),1], y1=confintindboot[k:(k-1+dim(x)[2]),2], code=3, angle=90, length=0.05, col="red"))
			suppressWarnings( arrows (x0=c(1:dim(x)[2]), y0=confintind[k:(k-1+dim(x)[2]),1], y1=confintind[k:(k-1+dim(x)[2]),2], code=3, angle=90, length=0.05, col="gray45"))
